...
const double p_grad = 100;   //dp/dx=const.
...
int main()
{
    ...
    for (int i=0; i<NX; i++){
        ...
        for (int j=1; j<NY; j++){
            double dudy = (mu/rho * (u_pre[j+1]-2*u_pre[j]+u_pre[j-1])/(dy*dy) - v_pre[j] * (u_pre[j+1]-u_pre[j-1])/(2*dy) - p_grad/rho);
    ...
    for (int i = 0; i < split_num+1; i++) {
        ...
        ss1 << "./p_grad/uv_x" << output_cols*dx << ".csv";
    ...
    int sep_index=-9;  //剥離した位置を記録する（発散値は回避する）
    //境界層厚さを求めて出力
    std::vector<double> delta;
    for (int i=0; i<NX+1; i++){
        int j=0;
        while (u[j][i] < u[NY][i]*BL_threshold){
            j++;
        }
    ...
    for (int i=0; i<NX+1; i++){
        ...
        int j=0;
        while (std::abs( 1 - u[j][i]/u[NY][i] ) > 0.0001){
            delta_star_pre += (1 - u[j][i]/u[NY][i]) * dy;
            j++;
        }
    ...
    for (int i=0; i<NX+1; i++){
        ...
        int j=0;
        while (std::abs( 1 - u[j][i]/u[NY][i] ) > 0.0001){
            theta_pre += u[j][i]/u[NY][i] * (1 - u[j][i]/u[NY][i]) * dy;
            j++;
        }
    ...
    for (int i=0; i<NX+1; i++){
        double Theta_pre=0;
        int j=0;
        while (std::abs( 1 - std::pow(u[j][i]/u[NY][i], 2) ) > 0.0001){
            Theta_pre += u[j][i]/u[NY][i] * (1 - std::pow(u[j][i]/u[NY][i], 2)) * dy;
            j++;
        }
        Theta.push_back(Theta_pre*1000);
        if (i==NX){
    ...
    for (int i=0; i<NX+1; i++){
        Cf.push_back(2/(rho*u[NY][i]*u[NY][i]) * (mu * (u[1][i]-u[0][i])/dy) * 1000);
        if (i==NX){
            ofs2 << Cf[i] << std::endl;
        }else{
            ofs2 << Cf[i] << ",";

...
const int i_1 = 171;   // x軸格子点数基準での，大体の翼の左端
const int i_2 = 231;  // x軸格子点数基準での，大体の翼の右端
// y-grid parameters
const int my = 401;   // y軸格子点数(1~401)
const int j_1 = 198;   // y軸格子点数基準での，大体の翼の下端
const int j_2 = 216;  // y軸格子点数基準での，大体の翼の上端
// set delta x,y,t
...
/* csv出力 */
int write_csv_p(double p[mx + 2][my + 2], const char *file_name) {
    FILE *fp;
...
int write_csv_cdclcp(double cd, double cl, double cp1, double cp2) {
    FILE *fp;
    fp = fopen("./airfoil/data/cdclcp_10.csv", "a");
    fprintf(fp, "%f,%f,%f,%f\n", cd, cl, cp1, cp2);
    fclose(fp);
    return 0;
}
...
int main() {
    std::vector<int> data1 = read_csv("./airfoil/joukowsky_10.csv");
    ...
    std::vector<int> data2 = read_csv("./airfoil/joukowsky_wall_10.csv");
    ...
    for (int n = 1; n <= nlast; n++) {
		...
		double cp1 = 2 * p[i_2 + i_2][j_1];
		double cp2 = 2 * p[i_2 + i_2][j_2];
        ...
        if (n % 50 == 0) {
            std::string s_p = "./airfoil/data/p_10_" + std::to_string(n) + ".csv";
            const char *cs_p = s_p.data();
            write_csv_p(cp, cs_p);
            std::string s_u = "./airfoil/data/u_10_" + std::to_string(n) + ".csv";
            const char *cs_u = s_u.data();
            write_csv_p(u, cs_u);
            std::string s_v = "./airfoil/data/v_10_" + std::to_string(n) + ".csv";
            const char *cs_v = s_v.data();
            write_csv_p(v, cs_v);
        }
		write_csv_cdclcp(cd, cl, cp1, cp2);
    }
